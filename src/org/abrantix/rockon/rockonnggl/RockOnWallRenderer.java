...
public class RockOnWallRenderer extends RockOnRenderer implements GLSurfaceView.Renderer {
	...
	public RockOnWallRenderer(Context context, Handler requestRenderHandler, int theme, int browseCat, INotifiableManager manager) {
		mContext = context;
	...
	public int[] getConfigSpec() {
		...
		int[] configSpec = { EGL10.EGL_RED_SIZE, 8, EGL10.EGL_GREEN_SIZE, 8, EGL10.EGL_BLUE_SIZE, 8, EGL10.EGL_ALPHA_SIZE, 8, EGL10.EGL_DEPTH_SIZE, 16,
				EGL10.EGL_NONE };
		return configSpec;
		// } else {
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		// gl.glFogx(GL10.GL_FOG_MODE, GL10.GL_EXP); // GL_EXP2 doesnt show
		// anything
	...
	public void onDrawFrame(GL10 gl) {
		...
		/*
		 * Upon browsing category change we want to clear the screen until the
		...
		/*
		 * Check if we are doing a click animation - it changes the eye spot of
		 * the camera
		 */
		if (mClickAnimation) {
			logisticFuncResult = (float) (2 * (1 / (1 + Math.pow(Math.E, -(float) this.mClickAnimationStep / (float) this.MAX_CLICK_ANIMATION_STEPS * 6))) - 1);
			/* camera eye update */
			...
			mCenterX = (float) (mCenterInitialX + (mCenterTargetX - mCenterInitialX) * logisticFuncResult);
			mCenterY = (float) (mCenterInitialY + (mCenterTargetY - mCenterInitialY) * logisticFuncResult);
			mCenterZ = (float) (mCenterInitialZ + (mCenterTargetZ - mCenterInitialZ) * logisticFuncResult);
	...
	void showClickAnimation(float x, float y) {
		...
		this.mCenterTargetY = -2 + 2 * getRowAndColumnFromScreenCoordinates(x, y)[1]; // duplicated
																						// effort
																						// --
																						// FIXME
	...
	private boolean updateTextures(GL10 gl) {
		...
		if (mCursor != null && !mCursor.isClosed()) {
			...
			for (int i = 0; i < mCacheSize; i++) {
				...
				albumIndexTmp = (int) Math.floor((flooredPositionY * 2) / mCacheSize) * mCacheSize + i;
				if (albumIndexTmp < flooredPositionY * 2 - 4) {
					albumIndexTmp += mCacheSize;
				}
				// should never happen
				else if (albumIndexTmp >= flooredPositionY * 2 - 4 + mCacheSize) {
					albumIndexTmp -= mCacheSize;
				}
	...
	private boolean setupAlbumTextures(GL10 gl, int cacheIndex, int navIndex, boolean force) {
		...
		if (mNavItem[cacheIndex].index != navIndex || force) {
			// Log.i(TAG,
			// "albumIndex: "+albumIndex+"/"+mAlbumCursor.getCount()+" flooredPositionY: "+flooredPositionY+" mPositionY: "+mPositionY);
			...
			if (navIndex < 0 || navIndex >= mCursor.getCount()) {
				...
				case Constants.BROWSECAT_ALBUM:
					if (!mNavItemUtils.fillAlbumInfo(mCursor, mNavItem[cacheIndex], navIndex)) {
					...
					if (!mNavItemUtils.fillAlbumBitmap(mNavItem[cacheIndex], mBitmapWidth, mBitmapHeight, mColorComponentBuffer, mTheme)) {
						...
						try {
							mNavItemUtils.fillAlbumUnknownBitmap(mNavItem[cacheIndex], mContext.getResources(), mNavItem[cacheIndex].cover.getWidth(),
									mNavItem[cacheIndex].cover.getHeight(), mColorComponentBuffer, mTheme);
				...
				case Constants.BROWSECAT_ARTIST:
					if (navIndex < mArtistAlbumHelper.length) {
						if (!mNavItemUtils.fillArtistInfo(mCursor, mNavItem[cacheIndex], mArtistAlbumHelper[navIndex], navIndex)) {
							mNavItem[cacheIndex].artistId = null;
							mNavItem[cacheIndex].artistName = null;
							mNavItem[cacheIndex].nAlbumsFromArtist = 0;
							mNavItem[cacheIndex].nSongsFromArtist = 0;
						}
						if (!mNavItemUtils.fillArtistBitmap(mNavItem[cacheIndex], mArtistAlbumHelper[navIndex], mBitmapWidth, mBitmapHeight,
								mColorComponentBuffer, mTheme)) {
							mNavItemUtils.fillAlbumUnknownBitmap(mNavItem[cacheIndex], mContext.getResources(), mNavItem[cacheIndex].cover.getWidth(),
									mNavItem[cacheIndex].cover.getHeight(), mColorComponentBuffer, mTheme);
						}
						...
						// mNavItem[cacheIndex].label.eraseColor(Color.argb(0,
						// 0, 0, 0));
						// }
					} else {
	...
	public void inertialScrollOnTouchEnd(float scrollSpeed, int direction) {
		...
		case Constants.SCROLL_MODE_VERTICAL:
			...
			// mPositionY
			// +
			// 0.5f * Math.signum(scrollSpeed/(mHeight*.5f)) // needs to be .5f
			...
			// else
			// {
			mTargetPositionY = Math.round(mPositionY + Constants.SCROLL_SPEED_BOOST * scrollSpeed / (mHeight * .5f));
			// }
	...
	public synchronized int getShownElementId(float x, float y) {
		if (mTargetPositionY != mPositionY || mCursor == null || mCursor.isClosed() ||
		/**
		...
		{
			// Log.i(TAG,
			// "Target was not reached yet: "+mTargetPosition+" - "+mPosition);
			return -1;
		} else {
			try {
				...
				if (mBrowseCat == Constants.BROWSECAT_ALBUM)
					id = mCursor.getInt(mCursor.getColumnIndexOrThrow(MediaStore.Audio.Albums._ID));
				else if (mBrowseCat == Constants.BROWSECAT_ARTIST)
					id = mCursor.getInt(mCursor.getColumnIndexOrThrow(MediaStore.Audio.Artists._ID));
	...
	public String getShownAlbumName(float x, float y) {
		if (mTargetPositionY != mPositionY)
			return null;
		else {
			int tmpIndex = mCursor.getPosition();
	...
	/** get the current Album Name */
	public String getShownAlbumArtistName(float x, float y) {
		if (mTargetPositionY != mPositionY)
			return null;
		else {
			int tmpIndex = mCursor.getPosition();
				...
				for (int i = 0; i < mCursor.getCount() - 1; i++) {
					...
					try {
						if (mCursor.getLong(mCursor.getColumnIndexOrThrow(MediaStore.Audio.Albums._ID)) == albumId) {
							...
							mPositionY = mTargetPositionY - Math.signum(mTargetPositionY - mPositionY)
									* Math.min(Math.abs(mTargetPositionY - mPositionY), 5.5f);
	...
	synchronized public void changeBrowseCat(int browseCat) {
		if (browseCat != mBrowseCat) {
			...
			this.renderNow();
			mCursor.close();
			initNonGlVars(mContext, browseCat, true, null);
			mBrowseCat = browseCat;
			mPositionY = -5.f;
	...
	private int[] mTextureId = new int[mCacheSize]; // the number of textures
	...
	private int[] mTextureLabelId = new int[mCacheSize]; // the number of
															...
															// number of faces
															// of our shape
	// private int[] mTextureAlphabetId = new int[mCacheSize]; // the number of
	// textures must be equal to the number of faces of our shape
...
class RockOnCover {
	...
	public RockOnCover() {
		 ...
		 * Texture coordinates
		 */
		float[] textCoords = { 0.f, 1.f, 1.f, 1.f, 1.f, 0.f, 0.f, 0.f };

		/**
		 * Generate our openGL buffers with the vertice and texture coordinates
		 * and drawing indexes VERTICAL
		 */
		...
		ByteBuffer vbb = ByteBuffer.allocateDirect(VERTS * 3 * 4); // verts *
																	// ncoords *
																	// bytes per
																	// vert??
