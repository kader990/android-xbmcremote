...
import org.xbmc.backend.async.thread.HttpApiDiskCacheThread;
import org.xbmc.backend.async.thread.HttpApiDownloadThread;
import org.xbmc.backend.async.thread.HttpApiMemCacheThread;
...
public class MusicWrapper extends Wrapper {
	...
	public void addToPlaylist(final HttpApiHandler<Boolean> handler, final Album album, final Song song) {
		mHandler.post(new Runnable() {
			public void run() { 
				try {
					final MusicClient mc = music(handler);
					final PlayStatus ps = control(handler).getPlayState();
	 ...
	 */
	private void checkForPlayAfterQueue(final MusicClient mc, final ControlClient cc, int numAlreadyQueued) throws BackendException {
