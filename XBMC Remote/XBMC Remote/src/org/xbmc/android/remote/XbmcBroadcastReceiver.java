...
import org.xbmc.backend.eventclient.Packet;
import org.xbmc.backend.exceptions.BackendException;
import org.xbmc.backend.httpapi.HttpClient;
...
public class XbmcBroadcastReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		try {
			http = ConnectionManager.getHttpClient(context);
		} catch (BackendException e1) {
			// if we can't even get the connection we stop doing ANYTHING
			e1.printStackTrace();
		}
			...
			try {
				if(action.equals(android.telephony.TelephonyManager.ACTION_PHONE_STATE_CHANGED)
					...
					if(extra.equals(android.telephony.TelephonyManager.EXTRA_STATE_RINGING)) {
	                    ...
	                    if(http != null) {
		                    ICurrentlyPlaying cp = null;
							try {
								cp = http.control.getCurrentlyPlaying();
